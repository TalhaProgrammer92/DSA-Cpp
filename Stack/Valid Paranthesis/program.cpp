...
bool is_valid_parenthesis(string paren)
{
	...
	for (char p : paren)
	{
		// ? Push opening parenthesis onto stack - Opening bracket case
		if (p == '(' || p == '[' || p == '{')
			parenthesis.push(p);
		// ? Pop recently added open parenthesis and check if its closing one exist in correct place - Closing bracket case
		else
		{
			// ! If closing bracket had no opening bracket
